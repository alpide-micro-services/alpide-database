...
CREATE TABLE IF NOT EXISTS `alpide-sales`.customer_ar_opening_balance (
    ...
    as_of_date                  DATETIME,
    ar_ledger_account_id        BIGINT,          -- GL Account 1100 (AR Control)
    ob_equity_ledger_account_id BIGINT,          -- GL Account 3900 (OB Equity)
...
-- Dual-ledger COA transaction entries for AR Opening Balance
-- Two rows per OB record: DEBIT AR (1100) + CREDIT OB Equity (3900)
-- Joined to customer_ar_opening_balance via (opening_balance_id, rid)

CREATE TABLE IF NOT EXISTS `alpide-sales`.customer_coa_tx_ar_opening_balance (
    ...
    REFERENCES customer_ar_opening_balance (opening_balance_id)
                                                                           ON DELETE CASCADE
    );

-- -----------------------------------------------------------------------------
-- 1. supplier_ap_opening_balance
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `alpide-purchase`.supplier_ap_opening_balance (
                                                                             opening_balance_id          BIGINT          NOT NULL AUTO_INCREMENT,
                                                                             version                     INT             NOT NULL DEFAULT 0,
                                                                             rid                         BIGINT          NOT NULL,
    ...
    PRIMARY KEY (opening_balance_id),
    UNIQUE KEY uq_ap_ob_rid_supplier (rid, supplier_id),
    INDEX idx_ap_ob_rid      (rid),
    INDEX idx_ap_ob_supplier (rid, supplier_id)
    );

-- -----------------------------------------------------------------------------
-- 2. supplier_coa_tx_ap_opening_balance
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `alpide-purchase`.supplier_coa_tx_ap_opening_balance (
                                                                                    ob_coa_tx_id                BIGINT          NOT NULL AUTO_INCREMENT,
                                                                                    version                     INT             NOT NULL DEFAULT 0,
                                                                                    opening_balance_id          BIGINT          NOT NULL,
                                                                                    rid                         BIGINT          NOT NULL,
    ...
    PRIMARY KEY (ob_coa_tx_id),
    INDEX idx_ap_coa_ob_id    (opening_balance_id),
    INDEX idx_ap_coa_rid      (rid),
    INDEX idx_ap_coa_supplier (rid, supplier_id),

    CONSTRAINT fk_ap_coa_opening_balance
    FOREIGN KEY (opening_balance_id)
    REFERENCES `alpide-purchase`.supplier_ap_opening_balance (opening_balance_id)
                                                                           ON DELETE CASCADE
    );