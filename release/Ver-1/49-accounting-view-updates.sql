...
CREATE  OR REPLACE
    ALGORITHM = UNDEFINED
    SQL SECURITY DEFINER
VIEW `alpide-sales`.`daybook_tx_vw` AS
SELECT
...
FROM
    `alpide-sales`.`customer_coa_tx_invoice` `inv_coa`
WHERE
    (`inv_coa`.`is_inactive` = 0)
UNION ALL SELECT
...
FROM
    `alpide-sales`.`customer_coa_tx_sales_order` `so_coa`
UNION ALL SELECT
...
FROM
    `alpide-sales`.`customer_coa_tx_payment` `cp_coa`
UNION ALL SELECT
...
FROM
    `alpide-sales`.`customer_coa_tx_credit_memo` `cm_coa`
UNION ALL SELECT
              `adv_coa`.`advance_payment_coa_tx_id` AS `coa_tx_id`,
              ...
              'Advance Payment' AS `transaction_name`
FROM
    `alpide-sales`.`customer_coa_tx_advance_payment` `adv_coa`
UNION ALL SELECT
...
FROM
    `alpide-sales`.`customer_refund_coa_tx` `ref_coa`;
